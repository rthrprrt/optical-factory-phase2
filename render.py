...
def LoadObj1(entity: Entity):
    global _sceneObjNodes

    # Charger le fichier OBJ
    obj = trimesh.load(entity.path, process=True)

    # Vérifier si c'est une scène
    if isinstance(obj, trimesh.Scene):
        # Extraire la première géométrie (vous pouvez ajuster pour charger toutes les géométries)
        geometries = list(obj.geometry.values())
        if len(geometries) > 0:
            obj = geometries[0]  # Prendre le premier objet
        else:
            raise ValueError(f"Aucune géométrie trouvée dans la scène pour {entity.path}")

    # Vérifier si le matériau existe
    if hasattr(obj.visual, 'material') and obj.visual.material is not None:
        print(f"Matériau chargé : {obj.visual.material}")

    # Convertir en mesh Pyrender
    objMesh = pyrender.Mesh.from_trimesh(obj, smooth=True)
